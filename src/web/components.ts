      ...
      children: [
        ...
        accordionItem.create('hiraResolver', {
          ...
          children: [
            ...
            input.number('resolverMaxVideoDurationMinutes', { label: '通用视频上限分钟', description: '平台未单独配置时使用；0 表示不限制。', defaultValue: secondsToMinutesValue(config.resolver.maxVideoDurationSeconds), rules: [{ min: 0, max: 360 }] }),
            createSwitch('resolverPlatformBilibili', '启用 B站解析', config.resolver.platforms.bilibili, '关闭后 B站链接交还给后续插件处理。'),
            createSwitch('resolverPlatformDouyin', '启用抖音解析', config.resolver.platforms.douyin, '抖音视频仅保留诊断和降级，不在本轮深修。'),
            createSwitch('resolverPlatformKuaishou', '启用快手解析', config.resolver.platforms.kuaishou, '开启后快手链接使用通用接口兜底解析。'),
            createSwitch('resolverPlatformWeibo', '启用微博解析', config.resolver.platforms.weibo, '开启后自动解析微博分享。'),
            createSwitch('resolverPlatformTieba', '启用贴吧解析', config.resolver.platforms.tieba, '开启后自动解析贴吧帖子和分享卡。'),
            createSwitch('resolverPlatformXiaoheihe', '启用小黑盒解析', config.resolver.platforms.xiaoheihe, '开启后自动解析小黑盒帖子。'),
            createSwitch('resolverPlatformXiaohongshu', '启用小红书解析', config.resolver.platforms.xiaohongshu, '开启后自动解析小红书笔记。'),
            createSwitch('resolverPlatformGeneral', '启用通用解析', config.resolver.platforms.general, '开启后西瓜、即刻、皮皮虾等通用平台使用兜底接口。'),
            createSwitch('resolverSendingContentForwardEnabled', '发送正文合并转发', config.resolver.sending.contentForwardEnabled, '开启后完整正文和图文顺序使用伪装合并转发。'),
            createSwitch('resolverSendingVideoFailureFallbackEnabled', '视频失败提示原链接', config.resolver.sending.videoFailureFallbackEnabled, '开启后视频发送失败时保留可读原因和原链接。'),
            createSwitch('resolverMediaDedupeImages', '图片资源去重', config.resolver.media.dedupeImages, '开启后同图不同尺寸只发送质量较高的一张。'),
            createSwitch('resolverMediaFilterLowQualityImages', '过滤低质量图片', config.resolver.media.filterLowQualityImages, '开启后过滤缩略图、模糊图和马赛克图。'),
            createSwitch('resolverMediaInlinePreviewCover', '预览封面内联', config.resolver.media.inlinePreviewCover, '开启后尽量内联预览卡封面，减少防盗链缺图。'),
            createSwitch('resolverDiagnosticsStageLogsEnabled', '输出解析阶段日志', config.resolver.diagnostics.stageLogsEnabled, '开启后记录 match、api、fallback、media、send 等阶段。'),
            createSwitch('resolverDiagnosticsVerboseLogsEnabled', '输出成功阶段日志', config.resolver.diagnostics.verboseLogsEnabled, '关闭后仅保留失败告警日志。'),
