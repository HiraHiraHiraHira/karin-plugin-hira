...
import { a as replyResolvedPost } from "./message-tlVwsfIp.js";
import { n as fetchText, t as fetchJson } from "./http-CckAyz66.js";
import { n as downloadFile, t as runFfmpeg } from "./ffmpeg-BDZMnFem.js";
import { n as createTempFilePath } from "./temp-CGGBIhnK.js";
...
const firstPlayAddrVideoUrl = (value) => {
	return firstVideoUrl(asObject$2(value)?.url_list);
};
...
const selectDouyinVideoUrl = (video, quality) => {
	return selectDouyinBitRateUrl(video.bit_rate, quality) || firstPlayAddrVideoUrl(video.play_addr) || firstPlayAddrVideoUrl(video.play_addr_h264) || firstPlayAddrVideoUrl(video.download_addr);
};
...
const matchResolver = (url) => {
	const hostname = hostnameOf(url);
	if (!hostname || isMihoyoDomain(hostname)) return void 0;
...
const imagesFromHtml = (html) => [...html.matchAll(/<img[^>]+src="([^"]+)"/g)].map((match) => match[1]);
...
const resolveTieba = async (url, generalApis) => {
...
const normalizeWeiboStatus = (url, status, comments = []) => {
	...
	const description = [stripHtml$1(status.text || ""), comments.length > 0 ? `热门评论\n${comments.join("\n")}` : ""].filter(Boolean).join("\n\n");
	return {
		platform: "weibo",
		displayName: "微博",
		title: stripHtml$1(status.text || "").slice(0, 60),
		description,
...
const resolveWeibo = async (url, cookie = "") => {
	...
	if (cookie) headers.Cookie = cookie;
	try {
		const status = (await fetchJson(`https://m.weibo.cn/statuses/show?id=${encodeURIComponent(id)}`, { headers })).data;
...
const parseTextEntities = (linkText) => {
	...
	try {
		...
		for (const entity of list) {
			...
			if (entity.type === "text" && entity.text) texts.push(String(entity.text).trim());
			if (entity.type === "img" && entity.url) pushUnique$1(images, String(entity.url));
			if (entity.type === "html" && entity.text) {
				const html = String(entity.text);
				const imageMatches = html.matchAll(/<img[^>]+data-original="([^"]+)"/g);
				for (const match of imageMatches) pushUnique$1(images, match[1]);
...
const selectNote = (payload) => {
	...
	const noteDetailMap = asObject(root?.noteDetailMap) ?? asObject(root?.state?.note?.noteDetailMap) ?? asObject(root?.note?.noteDetailMap);
	const firstDetail = noteDetailMap ? asObject(Object.values(noteDetailMap)[0]) : void 0;
...
const normalizeXiaohongshuNote = (pageUrl, payload) => {
	...
	for (const item of Array.isArray(stream?.h264) ? stream.h264 : []) pushUnique(videos, item?.masterUrl);
	for (const item of Array.isArray(stream?.h265) ? stream.h265 : []) pushUnique(videos, item?.masterUrl);
...
const resolveMatched = async (url) => {
...
const handleResolverMessage = async (e, next) => {
	...
	if (!matchResolver(url)) return next?.();
	await replyResolvedPost(e, await resolveMatched(url));
	return true;
};
//#endregion
//#region src/apps/resolvers.ts
const resolverReg = /^(?!\s*[\[{])[\s\S]*https?:\/\/[^\s"'<>]+/i;
...
//# sourceMappingURL=resolvers-2vRMtxFc.js.map