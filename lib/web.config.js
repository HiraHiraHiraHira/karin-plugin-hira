import { i as saveAllConfig, o as Root, r as readAllConfig } from "./config-BFMeS00C.js";
import { accordion, accordionItem, defineConfig, input, select, switchComponent } from "node-karin";
...
const createSwitch = (key, label, defaultSelected, description) => switchComponent.create(key, {
	label,
	description,
	startText: "开启",
	endText: "关闭",
...
const createOptionalStringInput = (key, label, defaultValue, description) => input.string(key, {
	label,
	description,
	defaultValue,
	isRequired: false
...
const createWebConfigComponents = () => {
	const config = readAllConfig();
	return [accordion.create("hiraConfig", {
		label: "Hira 插件配置",
		title: "Hira 插件配置",
		variant: "splitted",
		...
		children: [
			accordionItem.create("hiraApp", {
				...
				children: [
					...
					input.string("appReplyPrefix", {
						label: "回复前缀",
						description: "普通命令回复使用；多平台解析结果不会附加此前缀。",
						defaultValue: config.app.replyPrefix
					}),
					input.string("appHelpCommand", {
						label: "帮助命令",
						description: "用于展示插件帮助的主命令。",
						defaultValue: config.app.helpCommand
					}),
					input.string("appStatusCommand", {
						label: "状态命令",
						description: "用于查看插件运行状态的主命令。",
						defaultValue: config.app.statusCommand
					})
				]
			}),
			accordionItem.create("hiraMusic", {
				...
				children: [
					...
					select.create("musicDefaultSource", {
						label: "默认音乐源",
						description: "未指定平台时优先使用的搜索来源。",
						defaultValue: config.music.defaultSource,
						items: sourceItems
					}),
					...
					input.number("musicPageSize", {
						label: "每页数量",
						description: "多选点歌列表每页展示的歌曲数量。",
						defaultValue: String(config.music.pageSize),
						rules: [{
							min: 1,
							max: 20
						}]
					}),
					input.number("musicSessionTtlSeconds", {
						label: "会话有效期秒数",
						description: "多选列表保留多久可继续用序号选择。",
						defaultValue: String(config.music.sessionTtlSeconds),
						rules: [{
							min: 30,
							max: 3600
						}]
			...
			accordionItem.create("hiraResolver", {
				...
				children: [
					...
					input.number("resolverPriority", {
						label: "解析优先级",
						description: "修改后需重启 Karin 才能完全生效。",
						defaultValue: String(config.resolver.priority),
						rules: [{
							min: 0,
							max: 9999
						}]
					}),
					input.number("resolverBilibiliMaxVideoDurationMinutes", {
						label: "B站下载上限分钟",
						description: "超过上限仅返回视频信息；0 表示不限制。",
						defaultValue: secondsToMinutesValue(config.resolver.bilibili.maxVideoDurationSeconds),
						rules: [{
							min: 0,
							max: 360
						}]
					}),
					select.create("resolverBilibiliQuality", {
						label: "B站下载画质",
						description: "控制 B站视频下载的最高画质。",
						defaultValue: String(config.resolver.bilibili.quality),
						items: bilibiliQualityItems
					}),
					select.create("resolverBilibiliCodec", {
						label: "B站视频编码",
						description: "自动模式优先选择更省体积的现代编码；兼容性异常时可改为 AVC。",
						defaultValue: config.resolver.bilibili.codec,
						items: bilibiliCodecItems
					}),
					input.number("resolverDouyinMaxVideoDurationMinutes", {
						label: "抖音下载上限分钟",
						description: "超过上限仅返回作品信息；0 表示不限制。",
						defaultValue: secondsToMinutesValue(config.resolver.douyin.maxVideoDurationSeconds),
						rules: [{
							min: 0,
							max: 360
						}]
					}),
					select.create("resolverDouyinQuality", {
						label: "抖音视频清晰度",
						description: "控制抖音视频直链解析时请求的清晰度。",
						defaultValue: config.resolver.douyin.quality,
						items: douyinQualityItems
					}),
			...
			accordionItem.create("hiraRuntime", {
				...
				children: [
					...
					input.number("runtimeCleanupMaxAgeMinutes", {
						label: "保留分钟数",
						description: "临时文件保留多久后可被清理。",
						defaultValue: String(config.runtime.cleanupMaxAgeMinutes),
						rules: [{
							min: 1,
							max: 10080
						}]
					}),
					input.string("runtimeFfmpegPath", {
						label: "ffmpeg 路径",
						description: "用于合并 B站 DASH 视频和音频。",
						defaultValue: config.runtime.ffmpegPath
					}),
					input.string("runtimeFfprobePath", {
						label: "ffprobe 路径",
						description: "预留给后续媒体信息检测。",
						defaultValue: config.runtime.ffprobePath
					})
				]
			}),
			accordionItem.create("hiraExtra", {
				...
				children: [
					...
					input.string("aiApiBaseUrl", {
						label: "AI API 地址",
						description: "兼容 OpenAI 接口格式的 API 地址。",
						defaultValue: config.ai.apiBaseUrl
					}),
					input.string("aiModel", {
						label: "总结模型",
						description: "用于网页总结的模型名称。",
...
const flattenConfig = (config, prefix = "") => {
	...
	for (const [key, value] of Object.entries(config)) {
		...
		if (isRecord(value)) Object.assign(result, flattenConfig(value, nextKey));
...
const webConfig = defineConfig({
	...
	save: async (patch) => {
		const next = mergeConfigPatch(readAllConfig(), patch);
		saveAllConfig(next);
		return {
			success: true,
			message: "Hira 配置已保存",
