...
import { d as buildStatusCardHtml, f as logCardRenderFailure, o as replyText, p as renderCardImage } from "../message.js";
import { t as cleanupRuntimeTemp } from "../temp.js";
import karin from "node-karin";
//#region src/apps/status.ts
const statusReg = createConfiguredCommandRegExp(Config.app.statusCommand, ["hira\\s*状态", "hi\\s*状态"]);
...
const status = karin.command(statusReg, async (e, next) => {
	if (!isHiraAppEnabled()) return next?.();
	await replyStatus(e);
	return true;
}, { name: "Hira-状态" });
const cleanup = karin.command(cleanupReg, async (e, next) => {
	if (!isHiraAppEnabled()) return next?.();
