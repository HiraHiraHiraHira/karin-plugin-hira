...
export const createConfigFieldRenderers = ({
  ...
  const renderTextField: ConfigFieldRenderers['renderTextField'] = (path, label, help, options) => {
    ...
    const rawValue = getValue(config, path, type === 'number' ? options?.fallback ?? 0 : '')
    const value = String(options?.format ? options.format(rawValue) : rawValue)
      ...
      <TextField
        ...
        onChange={(nextValue) => {
          const parsedValue = options?.parse
            ? options.parse(nextValue)
            : type === 'number'
              ? toNumber(nextValue, options?.fallback ?? 0)
              : nextValue
  ...
  const renderListField: ConfigFieldRenderers['renderListField'] = (path, label, help, options) => {
    ...
    const fieldElement = (
      <div className={classes.field}>
        <Label className="font-semibold">{label}</Label>
        <textarea
          className="min-h-28 w-full resize-y rounded-large border border-default-200 bg-default-50 px-3 py-2 text-sm outline-none transition-colors focus:border-primary disabled:cursor-not-allowed disabled:opacity-50"
          disabled={disabled}
          name={path.join('.')}
          placeholder={options?.placeholder || label}
          value={value}
